...
public class CustomLogWindow : EditorWindow
{
    ...
    private bool showWebRequests = false;
    private bool showTextureDictActivity = false;
    ...
    private static CustomLogWindow window;
    public static RequestMadeEventListener _rmel;

    public CustomLogWindow(ref RequestMadeEventListener listener)
    {
        _rmel = listener;
    }
...
[MenuItem("Window/Resizable Panels")]
    ...
    {
        window = (CustomLogWindow) EditorWindow.GetWindow(typeof(CustomLogWindow));
        window.titleContent = new GUIContent("PokeTCG Rest API Console");
    }
    ...
    private void OnEnable()
    {
        ...
        // TODO make my own custom Application.logMessageReceived but in GET_Caller Editor then subscribe
        _rmel = new RequestMadeEventListener();
    ...
    {
        Debug.Log("Custom Console Closed");
        Application.logMessageReceived -= LogMessageReceived;
    }
    ...
    private void DrawMenuBar()
    {
        ...
        GUILayout.Button(new GUIContent("Clear"), EditorStyles.toolbarButton, GUILayout.Width(50));
        GUILayout.Space(5);
 
        //collapse = GUILayout.Toggle(collapse, new GUIContent("Collapse"), EditorStyles.toolbarButton, GUILayout.ExpandWidth(true));
        //clearOnPlay = GUILayout.Toggle(clearOnPlay, new GUIContent("Clear On Play"), EditorStyles.toolbarButton, GUILayout.ExpandWidth(true));
        errorPause = GUILayout.Toggle(errorPause, new GUIContent("Error Pause"), EditorStyles.toolbarButton, GUILayout.ExpandWidth(true));
 
        GUILayout.FlexibleSpace();
 
        showWebRequests = GUILayout.Toggle(showWebRequests, new GUIContent("Web Requests", infoIconSmall), EditorStyles.toolbarButton, GUILayout.Width(100));
        showTextureDictActivity = GUILayout.Toggle(showTextureDictActivity, new GUIContent("Texture Dict Activity", warningIconSmall), EditorStyles.toolbarButton, GUILayout.Width(150));
        showErrors = GUILayout.Toggle(showErrors, new GUIContent("E", errorIconSmall), EditorStyles.toolbarButton, GUILayout.Width(45));
    ...
    private void LogMessageReceived() // void version for testing
    {
        Debug.Log("LogMessageReceived() called");
    }
